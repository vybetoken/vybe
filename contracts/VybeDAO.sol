...
contract VybeDAO is ReentrancyGuard {
    ...
    constructor(address stake) public {
    ...
    function _createNewProposal(ProposalType pType)
        internal
        active
        returns (uint64)
    {
    ...
    function proposeStakeUpgrade(
        address newStake,
        address[] calldata owned,
        string calldata info
    ) external returns (uint64) {
        ...
        _stakeUpgradeProposals[proposalID] = StakeUpgradeProposal(
            newStake,
            owned,
            info
        );
    ...
    function proposeDAOUpgrade(address newDAO, string calldata info)
        external
        returns (uint64)
    {
    ...
    function addVote(uint64 proposalID)
        external
        active
    ...
    function removeVote(uint64 proposalID)
        external
        active
        pendingProposal(proposalID)
    {
        ...
        noReentrancy
    {
        ...
        if (
            ...
            (meta.pType == ProposalType.StakeUpgrade) ||
            (meta.pType == ProposalType.DAOUpgrade)
        ) {
        ...
        if (meta.pType == ProposalType.Fund) {
            ...
            _stake.removeModule(_moduleRemovalProposals[proposalID].module);
             ...
             = _stakeUpgradeProposals[proposalID];
            for (uint256 i = 0; i < proposal.owned.length; i++) {
    ...
    function withdrawProposal(uint64 proposalID)
        external
        active
        pendingProposal(proposalID)
    {
