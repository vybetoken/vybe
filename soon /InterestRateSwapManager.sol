...
contract InterestRateSwapManager is Ownable {
    ...
    function offerLend(
        ...
        uint256 weeklyInterestDivisor,
        address arbitrator,
        uint256 expectedArbitrationFee,
        bytes4 expectedUseID
    ...
    function calculateBorrowFee(BorrowOffer memory borrow)
        internal
        view
        returns (uint256)
    {
        return
            borrow.amount.mul(borrow.lengthInWeeks).div(
                lends[borrow.lendID].weeklyInterestDivisor
            );
    }
    ...
    function offerBorrow(
        ...
        if (registeredIDs[lend.useID]) {
            bytes32 hash;
            assembly {
                hash := extcodehash(collector)
            }
            require(hash == verifiedContracts[lend.useID]);
            require(
                IInterestRateCollector(collector).token() == address(lend.token)
            );
            require(
                IInterestRateCollector(collector).manager() == address(this)
            );
    ...
    function cancelBorrow(uint256 id) external {
        ...
        require(
            lends[borrow.lendID].token.transfer(
                msg.sender,
                calculateBorrowFee(borrow)
            )
        );
    }
    ...
    function accept(uint256 borrowID) external returns (uint256 res) {
        ...
        LendOffer storage lend = lends[borrow.lendID];
        require(
            (msg.sender == lend.creator) || (msg.sender == lend.arbitrator)
        );
    ...
    function complete(uint256 id) external {
        ...
        require(
            swap.started + (borrow.lengthInWeeks.mul(1 weeks)) <=
                block.timestamp
        );
