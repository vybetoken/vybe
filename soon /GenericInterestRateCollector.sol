...
contract GenericInterestRateCollector is IInterestRateCollector {
    ...
    constructor(
        address owner,
        address token,
        address[] memory setupAddrs,
        bytes[] memory setup,
        bytes[] memory finishAddrs,
        bytes[] memory finish
    ) public {
        ...
        for (uint256 s = 0; s < setup.length; s++) {
            start[s] = Call(setupAddrs[s], setup[s]);
        }

        end = new Call[](finish.length);
        for (uint256 s = 0; s < finish.length; s++) {
    ...
    function _start() external {
        ...
        for (uint256 c = 0; c < start.length; c++) {
            start[c].contractAddr.call(start[c].data);
        }
    }

    function _end() external override {
        require(msg.sender == _owner);
        require(!ended);
        ended = true;

        for (uint256 c = 0; c < end.length; c++) {
